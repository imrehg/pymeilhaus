...
#include <Python.h>
//#include <numpy/arrayobject.h>   //wrong location of arrayobject.h - numpy for python-2.7.3 is not there, see below

//the following include applies to the installation of python-2.7.3.msi (32 bit) -> in C:\Python27.
//numpy-1.6.1-win32-superpack-python2.7.exe installs arrayobject.h in the folder below.
...
static me_const_info_t me_const_table[] = {
    ...
    { ME_PY_INT, "ME_SINGLE_CONFIG_FIO_INPUT", (long) ME_SINGLE_CONFIG_FIO_INPUT, 0 },                  //support for frequency input
    { ME_PY_INT, "ME_SINGLE_CONFIG_FIO_OUTPUT", (long) ME_SINGLE_CONFIG_FIO_OUTPUT, 0 },                //support for frequency output
...
static PyObject *_wrap_meIOSingleTimeToTicks(PyObject *self, PyObject *args) {
	int iDevice;
	int iSubdevice;
	int iTimer;
	double dTime;
	...
	if(!PyArg_ParseTuple(args, "iiidi:meIOSingleTimeToTicks", &iDevice, &iSubdevice, &iTimer, &dTime, &iFlags)) return NULL;
    
	iResult = meIOSingleTimeToTicks(iDevice, iSubdevice, iTimer, &dTime, &iTicksLow, &iTicksHigh, iFlags);
	if(iResult){
...
static PyObject *_wrap_meIOSingleTicksToTime(PyObject *self, PyObject *args) {
    int iDevice;
    int iSubdevice;
    int iTimer;
    int iTicksLow;
    ...
    if(!PyArg_ParseTuple(args, "iiiiidi:meIOSingleTicksToTime", &iDevice, &iSubdevice, &iTimer, &iTicksLow, &iTicksHigh, &dTime, &iFlags)) return NULL;
    
    iResult = meIOSingleTicksToTime(iDevice, iSubdevice, iTimer, iTicksLow, iTicksHigh, &dTime, iFlags);
	if(iResult){
...
static PyMethodDef meMethods[] = {
    ...
    { "meIOSingleTimeToTicks", _wrap_meIOSingleTimeToTicks, METH_VARARGS },             //meIOSingleTimeToTicks
    { "meIOSingleTicksToTime",  _wrap_meIOSingleTicksToTime, METH_VARARGS },            //meIOSingleTicksToTime
